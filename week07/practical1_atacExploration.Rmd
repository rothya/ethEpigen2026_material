...
Here we see the region of a motif - and the red graph shows you the occurance of ATAC fragments. 

mono_center: we have two peaks corresponding to the well positioned nucleosomes right left and right to the motifs > the TF will push the nucleosomes to both sides. 

NF: if the TF binds the motif > prevents Tn5 to insert there - resulting in this low coverage in the center. TF footprint. Some TF have a long residence time (= resulting in the strongest footprint) - several other TF have a low resience (will only stay bound for ms). The extent to which a TF leaves a footprint > gives you information about the residence time. 
By looking at footprint for factors we don't know yet > can reveal motifs.
...
mono_clusters: downstream the nucleosomes are not very well positioned > but if you go upstream, you have nucleosome peaks very clearly.
